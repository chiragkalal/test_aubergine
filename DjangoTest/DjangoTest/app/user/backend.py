...
class JWTAuthentication(authentication.BaseAuthentication):
    ...
    def _authenticate_credentials(self, request, token):
        ...
        try:
            ...
            if 'RefreshToken' not in request.headers.keys():
                raise exceptions.AuthenticationFailed('Refresh token is missing.')
            elif request.headers['Refreshtoken'] == "":
                raise exceptions.AuthenticationFailed('Refresh token should not be blank.')
            refresh_token = request.headers['Refreshtoken'].split(' ')[1]
